...
ParsoidService.init = Promise.method(function (parsoidOptions, processLogger) {
    ...
    if (parsoidConfig.allowCORS) {
        app.use(function (req, res, next) {
            ...
            res.set('Access-Control-Allow-Headers', 'Api-User-Agent');
            next();
        });
    }
    ...
    app.get('/:domain/v3/page/:format/:title/:revision?', v3, u, p, a,
        routes.v3Get);
    app.post('/:domain/v3/transform/:from/to/:format/:title?/:revision?', v3, u,
        p, a, routes.v3Post);
        ...
        return {
            close: function () {
                ...
                .tap(function () {
                    ...
                    require(
                        '../config/MWParserEnvironment.js').MWParserEnvironment.resetConfCache();
                });
            },
            port: port,
        };
